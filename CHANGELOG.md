...
11. ~~No hay un flujo formal de consentimiento informado dentro del producto~~ **Resuelto (pantalla) 9 ago, PR #34:** `vita-demo-formulario` ahora exige aceptar una pantalla de consentimiento antes de cualquier captura. **Falta un paso manual para que quede registrado en el Sheet** (columna `consentimiento_ts` ya preparada en `apps-script-formulario-v2.gs.txt`, pendiente de que alguien despliegue la nueva versión del Apps Script — no lo puede hacer un agente sin acceso al editor).
12. **Rate limiting real (Cloudflare KV) sigue bloqueado, y no solo por scope** (ver punto 6) — en esta sesión de trabajo autónomo no había ningún token de Cloudflare disponible en absoluto, ni siquiera el de alcance mínimo. Necesita que Leonardo lo provea o lo ejecute él mismo desde el dashboard.
13. **Política de retención/borrado de datos en Sheets** (ver punto 5): la decisión de negocio (cuánto tiempo, qué pasa al pedir borrado) sigue sin tomarse — es lo que hay que resolver primero, antes de escribir cualquier código de purga automática. Detalle y razonamiento completo: Notion "[be360 — Camino a TRL 9](https://app.notion.com/p/3b77dac3d7cc81928514d61dc2ca1fe9)".
